-- AI Content Moderation System - Database Migration
...
ADD COLUMN IF NOT EXISTS auto_moderation_status ENUM('pending', 'approved', 'rejected', 'flagged', 'reviewing') DEFAULT 'pending' COMMENT 'AI自動審核狀態',
ADD COLUMN IF NOT EXISTS auto_moderation_score DECIMAL(5, 2) DEFAULT NULL COMMENT 'AI審核信心度分數(0-100)',
ADD COLUMN IF NOT EXISTS auto_category_id INT DEFAULT NULL COMMENT 'AI建議的分類',
ADD COLUMN IF NOT EXISTS moderation_reason TEXT DEFAULT NULL COMMENT 'AI審核原因或標記說明',
ADD COLUMN IF NOT EXISTS needs_manual_review BOOLEAN DEFAULT FALSE COMMENT '是否需要人工審核',
ADD COLUMN IF NOT EXISTS reviewed_by INT DEFAULT NULL COMMENT '人工審核者ID',
ADD COLUMN IF NOT EXISTS reviewed_at TIMESTAMP NULL COMMENT '人工審核時間',
ADD INDEX idx_auto_moderation (auto_moderation_status),
ADD INDEX idx_manual_review (needs_manual_review),
ADD FOREIGN KEY fk_auto_category (auto_category_id) REFERENCES categories(id) ON DELETE SET NULL,
ADD FOREIGN KEY fk_reviewed_by (reviewed_by) REFERENCES users(id) ON DELETE SET NULL;
